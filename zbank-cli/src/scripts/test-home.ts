...
async function testHomeScreenFunctionality() {
  ...
  console.log(`Initial balance for ${testAccount}: ${formatBalance(initialBalance)}`);

  // Test deposit
  console.log('\n--- Testing Deposit ---');
  const depositAmount = 5000; // $50.00
  const depositResult = await transactionService.deposit(testAccount, depositAmount);
  ...
  const withdrawResult = await transactionService.withdraw(testAccount, withdrawAmount);
  ...
  const overdraftResult = await transactionService.withdraw(testAccount, overdraftAmount);
  
  if (overdraftResult.success) {
    console.log(`✓ Overdraft withdraw successful: ${formatBalance(overdraftAmount)}`);
