...
export class JsonStorage implements IStorage {
	...
	private async saveAccounts(
		accounts: Record<string, Account> | Map<string, Account>,
	): Promise<void> {
		...
		try {
			...
			await fs.writeFile(
				tempFile,
				JSON.stringify(accountsObj, null, 2),
				'utf-8',
			);
			...
			throw new Error(
				`Failed to save accounts: ${error instanceof Error ? error.message : String(error)}`,
				{
					cause: error,
				},
			);
