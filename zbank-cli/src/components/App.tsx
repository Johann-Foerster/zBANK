	...
	const renderScreen = () => {
		switch (currentState) {
			case AppState.SPLASH:
				return <SplashScreen onComplete={() => navigateTo(AppState.LOGIN)} />;
			...
			case AppState.HOME:
				return <HomeScreen onLogout={() => navigateTo(AppState.EXIT)} />;
			...
			case AppState.HELP:
				return <HelpScreen onBack={() => navigateTo(AppState.LOGIN)} />;
			...
			default:
				return (
					<Box>
						<Text>Unknown state: {currentState}</Text>
					</Box>
				);
		}
	};
