	...
	const handleTransactionSubmit = async () => {
		...
		try {
			...
			switch (selectedAction) {
				case 'deposit':
					result = await transactionService.deposit(
						account.accountNumber,
						amountInCents,
					);
					break;
				case 'withdraw':
					result = await transactionService.withdraw(
						account.accountNumber,
						amountInCents,
					);
					break;
			...
			if (result.success && result.newBalance !== undefined) {
				...
				setMessage(
					`Transaction successful! New balance: ${formatBalance(result.newBalance)}`,
				);
				setAmount('');
		...
		<Box flexDirection="column">
			...
			<Box flexDirection="column" paddingX={2}>
				...
				<Box flexDirection="column" marginBottom={1}>
					<Text>
						Welcome, Account: <Text color="cyan">{account.accountNumber}</Text>
					</Text>
